...
class LookupVar:
  '''A symbol table for looking up the values of variables
  while evaluating an expression. Raises EvalError if the
...
class Expr:
  ...
  def pprint_prec(self, prec: int) -> str:
    '''Pretty-print an expression nested as a parameter of
    an operator with the given precedence.'''
    # By default ignore the precedence, unless this is
    # itself an operator.
    return self.pprint()
...
@dataclass
class ScalarVar(Var):
  '''A reference to a scalar variable in an expression.'''
  def pprint(self) -> str:
    return self.name
...
class ArrayVar(Var):
  ...
  def evaluate(self, lv: LookupVar) -> float:
    return lv.lookup_array(
      ArrayElt(name=self.name, subscr=self.subscr.evaluate(lv)))
